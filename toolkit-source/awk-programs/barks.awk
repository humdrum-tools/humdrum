...
BEGIN {
	...
	indicators = "^(\\*\\+|\\*\\-|\\*\\^|\\*v|\\*x)$"
	input_interps = "^(\\*\\*cbr|\\*\\*cocho|\\*\\*freq|\\*\\*barks)$"
	#
...
function parse_command()
	...
	if (ARGV[2] != "null")
		...
		if (ARGV[2] !~ /^[0-9]*$/)
			{
			print "barks: ERROR: Invalid precision specified: " \
				 ARGV[2] > stderr
			print USAGE > stderr
			exit
			}
...
function store_new_interps(      j,interp_line)
	...
	for (j = 1; j <= current_no_of_spines; j += 1)
		...
		# the output line must show barks.
		#
		if ($j ~ input_interps) interp_line = interp_line "**barks\t"
	...
	for (j = 1; j <= current_no_of_spines; j += 1)
		...
		if (current_interp[j] ~ input_interps)
			...
			#
				...
				# interpretation in order to obtain barks output.
				#
				...
				else if (current_interp[j] == "**barks")
					current_token = process_barks($j)
				}
			#
...
########################################################################
##				Function Convert_to_barks
#	This function accepts a frequency value from all the functions and
# converts it to barks.
#
function convert_to_barks(freq,   fkhz,barks)
	{
	fkhz = freq / 1000.
	barks = (13.0*atan2(0.76*fkhz,2.)+3.5*atan2(fkhz*fkhz/56.25,2.))
	return (sprintf("%." precision "f",barks))
	}

#############################################################################
##				Function Process_freq
#	This function takes frequency input and translates to barks
#
	...
	for (j = 1; j <= split_num; j += 1)
		...
		#
			...
			if (match(arrayn[j],floating_pt_num))
				...
				#if (freq < -228)
				#	{
				#	print "barks: ERROR: Invalid negative freq value in "\
				#		 "line " NR "." > stderr
				#	exit
				#	}
				freq = convert_to_barks(freq)
		...
		#
			...
			if (match(arrayf[j],floating_pt_num))
				...
				if (cbr < -4600 || cbr > 4600)
					{
					print "barks: ERROR: cbr value out of range in "\
						 "line " NR "." > stderr
					exit
					}
...
#	This function takes cocho input and translates to barks
#
		...
		#
			...
			if (match(arrayg[j],floating_pt_num))
				...
				if (cocho < -3600 || cocho > 3600)
					{
					print "barks: ERROR: Cocho value out of range in "\
						 "line " NR "." > stderr
					exit
					}
...
######################################################################
##				Function Process_barks
#	This function takes barks input and translates to barks
#
function process_barks(data_token,     barks,return_token,arrayk,j,split_num)
	{
	#
	# Process multiple stops
	#
	...
	for (j = 1; j <= split_num; j += 1)
		...
		#
		# Otherwise, process the barks value
		#
		else
			{
			barks = 0
			#
			# If a valid barks exists, store an process it
			#
			if (match(arrayk[j],floating_pt_num))
				{
				barks = substr(arrayk[j],RSTART,RLENGTH) + 0
				sub(floating_pt_num,SUBSEP,arrayk[j])
				barks = sprintf("%." precision "f",barks)
				sub(SUBSEP,barks,arrayk[j])
				return_token = return_token arrayk[j] " "
				}
			#
			# If no bark present, output depends on -x option
			#
			else return_token = return_token arrayk[j] " "
			}
		}
	#
	# At least return a null token
