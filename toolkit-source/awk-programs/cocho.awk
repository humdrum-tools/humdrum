...
#	exchange_spines()		process_data()			convert_to_cocho()
...
##################################################################
##			FUNCTIONS USED IN COCHO.AWK
##################################################################

##################################################################
##			Function Parse_command
#	This function checks that the input passed from cocho.ksh
...
function parse_command()
	...
	if (ARGV[2] != "null")
		...
		if (ARGV[2] !~ /^[0-9]*$/)
			{
			print "cocho: ERROR: Invalid precision specified: " \
				 ARGV[2] > stderr
			print USAGE > stderr
			exit
			}
...
function store_new_interps(      j,interp_line)
	...
	for (j = 1; j <= current_no_of_spines; j += 1)
		...
		# the output line must show cocho.
		#
		if ($j ~ input_interps) interp_line = interp_line "**cocho\t"
	...
	for (j = 1; j <= current_no_of_spines; j += 1)
		...
		if (current_interp[j] ~ input_interps)
			...
			#
				...
				# interpretation in order to obtain cocho output.
				#
...
#######################################################################
##				Function Convert_to_cocho
#	This function takes the semits produced from each function and 
# converts them to cochlear coordinates.
#
function convert_to_cocho(frequency)
	{
	return \
		(sprintf("%." precision "f",((log(frequency/165 + 1))/log(10))/0.06))
	}

#############################################################################
##				Function Process_freq
#	This function takes frequency input and translates to cocho
#
function process_freq(data_token,   j,return_token,split_num,arrayn,freq,cocho)
	{
	#
	...
	for (j = 1; j <= split_num; j += 1)
		...
		#
			...
			if (match(arrayn[j],floating_pt_num))
				...
				if (freq < -160)
					{
					print "cocho: ERROR: Invalid negative freq value in"\
						 " line " NR "." > stderr
					exit
					}
				cocho = convert_to_cocho(freq)
				sub(SUBSEP,cocho,arrayn[j])
...
#	This function takes cbr input and translates to cocho
#
function process_cbr(data_token, cbr,freq,return_token,arrayf,j,split_num,cocho)
	{
	#
	# Process multiple stops
	#
	...
	for (j = 1; j <= split_num; j += 1)
		...
		#
			...
			if (match(arrayf[j],floating_pt_num))
				...
				if (cbr < -11 || cbr > 4600)
					{
					print "cocho: ERROR: cbr value out of range in "\
						 "line " NR "." > stderr
					exit
					}
				freq = ((10 ^ (cbr / 21.4) - 1) / 4.37) * 1000
				cocho = convert_to_cocho(freq)
				sub(SUBSEP,cocho,arrayf[j])
...
#	This function takes cocho input and translates to cocho
#
function process_cocho(data_token,     cocho,return_token,arrayg,j,split_num)
	{
	#
	# Process multiple stops
	#
	...
	for (j = 1; j <= split_num; j += 1)
		...
		#
			...
			if (match(arrayg[j],floating_pt_num))
				...
				cocho = sprintf("%." precision "f",cocho)
				sub(SUBSEP,cocho,arrayg[j])
