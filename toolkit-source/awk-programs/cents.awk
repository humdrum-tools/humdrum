...
##################################################################
##			FUNCTIONS USED IN CENTS.AWK
##################################################################

##################################################################
##			Function Parse_command
#	This function checks that the input passed from cents.ksh
...
function parse_command()
	...
	if (ARGV[3] != "null")
		...
		if (ARGV[3] !~ /^[0-9]*$/)
			{
			print "cents: ERROR: Invalid precision specified: " \
				 ARGV[3] > stderr
			print USAGE > stderr
			exit
			}
...
function store_new_interps(      j,interp_line)
	...
	for (j = 1; j <= current_no_of_spines; j += 1)
		...
		# the output line must show cents.
		#
		if ($j ~ input_interps) interp_line = interp_line "**cents\t"
...
function get_absolute(interp,position,    temp_index)
	...
	if (interp !~ abs_tuning)
		{
		print "cents: ERROR: Invalid absolute tuning interpretation in "\
			 "line " NR "." > stderr
		exit
		}
	#
...
function get_relative(interp,position,   courses,i,temp_index,num1,string_num)
	...
	if (interp !~ rel_tuning)
		{
		print "cents: ERROR: Invalid relative tuning interpretation in "\
			 "line " NR "." > stderr
		exit
		}
	#
...
function get_frets(interp,position,   temp_index,temp_array,num1,frets,i)
	...
	if (interp !~ fret_tuning)
		{
		print "cents: ERROR: Invalid fret tuning interpretation in "\
			 "line " NR "." > stderr
		exit
		}
	#
	...
	for (j = 1; j <= current_no_of_spines; j += 1)
		...
		if (current_interp[j] ~ input_interps)
			...
			#
				...
				# interpretation in order to obtain cents output.
				#
...
#	This function takes kern input and translates to cents
#
		...
		#
			...
			if (found)
				...
				if (match(arrayc[j],/#+/))
					{ semits += RLENGTH ; sub(/#+/,"",arrayc[j]) }
				gsub("n","",arrayc[j])
				semits = sprintf("%." precision "f",semits*100)
...
#	This function accepts pitch input and translates to cents 
#
...
#	This function takes Tonh input and translates to cents
#
...
#	This function takes solfg input and translates to cents
#
...
#	This function takes specC input and translates to cents
#
		...
		#
			...
			if (match(arraym[j],floating_pt_num))
				...
				if (specC > 0)
					...
					{
					print "cents: ERROR: Invalid negative specC value in"\
						 " line " NR "." > stderr
					exit
					}
				semits = sprintf("%." precision "f",semits*100)
...
#	This function takes frequency input and translates to cents
#
		...
		#
			...
			if (match(arrayn[j],floating_pt_num))
				...
				if (freq > 0)
					...
					{
					print "cents: ERROR: Invalid negative freq value in"\
						 " line " NR "." > stderr
					exit
					}
				semits = sprintf("%." precision "f",semits*100)
...
#	This function takes semits input and translates to cents
#
...
#	This function takes cents input and translates to cents
#
	...
	for (j = 1; j <= split_num; j += 1)
		...
		#
			...
			if (match(arraye[j],floating_pt_num))
				...
				cents = sprintf("%." precision "f",cents)
...
#	This function takes MIDI input and translates to cents
#
function process_MIDI(data_token,    note,arrayk,j,return_token,split_num\
	...
	for (j = 1; j <= split_num; j += 1)
		...
		if (note > 0)
			...
			note -= 60
			return_token \
					= return_token sprintf("%." precision "f",note*100) " "
			}
		}
	#
	# At least return a null token
...
#	This function takes fret input and translates to cents
#
		...
		for (j = 1; j <= split_num; j += 1)
			...
			if (arrayl[j] ~ /[^x:-]/)
				...
				if (!((temp_index,j) in relative))
					{
					print "cents: ERROR: No pitch specified for string "\
				...
				for (i = 1; i <= split(relative[temp_index,j],strings,",");\
					...
					if (semits != old_semits || return_token == "")
						return_token = return_token \
							sprintf("%." precision "f",semits*100) " "
					old_semits = semits
					}
				}
			}
