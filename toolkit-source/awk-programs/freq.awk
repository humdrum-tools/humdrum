...
# June 23/94	Tim Racinsky	Changed solfg representation to contain "-"
#
#
#	This program is used to convert pitch units encoded  in various
# representations to frequency.
#
# Main Functions used:
...
#  -precision: holds precision of output
#
#	ARRAYS:
#
...
##################################################################
##			FUNCTIONS USED IN FREQ.AWK
##################################################################

##################################################################
##			Function Parse_command
#	This function checks that the input passed from freq.ksh
...
function parse_command()
	...
	if (ARGV[3] != "null")
		...
		if (ARGV[3] !~ /^[0-9]*$/)
			{
			print "freq: ERROR: Invalid precision specified: " \
				 ARGV[3] > stderr
			print USAGE > stderr
			exit
			}
...
function store_new_interps(      j,interp_line)
	...
	for (j = 1; j <= current_no_of_spines; j += 1)
		...
		# the output line must show freq.
		#
		if ($j ~ input_interps) interp_line = interp_line "**freq\t"
...
function get_absolute(interp,position,    temp_index)
	...
	if (interp !~ abs_tuning)
		{
		print "freq: ERROR: Invalid absolute tuning interpretation in "\
			 "line " NR "." > stderr
		exit
		}
	#
...
#########################################################################
##					Function Get_semits
#  This function simply translates pitch_array into semits without any checking
#
...
function get_relative(interp,position,   courses,i,temp_index,num1,string_num)
	...
	if (interp !~ rel_tuning)
		{
		print "freq: ERROR: Invalid relative tuning interpretation in "\
			 "line " NR "." > stderr
		exit
		}
	#
...
function get_frets(interp,position,   temp_index,temp_array,num1,frets,i)
	...
	if (interp !~ fret_tuning)
		{
		print "freq: ERROR: Invalid fret tuning interpretation in "\
			 "line " NR "." > stderr
		exit
		}
	#
	...
	for (j = 1; j <= current_no_of_spines; j += 1)
		...
		if (current_interp[j] ~ input_interps)
			...
			#
				...
				# interpretation in order to obtain frequency output.
				#
				...
				else if (current_interp[j] == "**cocho")
...
########################################################################
##				Function Convert_to_freq
#	This function accepts a semits value from all the functions and
# converts it to frequency.
#
function convert_to_freq(semits)
	{
	return (sprintf("%." precision "f",middle_C_freq * (1.059463094 ^ semits)))
	}

####################################################################
##				Function Process_kern
#	This function takes kern input and translates to frequency
#
...
#	This function accepts pitch input and translates to frequency 
#
...
#	This function takes Tonh input and translates to frequency
#
...
#	This function takes solfg input and translates to frequency
#
...
#	This function takes specC input and translates to frequency
#
...
#	This function takes frequency input and translates to frequency
#
...
#	This function takes semits input and translates to frequency
#
...
#	This function takes cents input and translates to frequency
#
function process_cents(data_token,    arraye,j,return_token,cents,split_num)
	{
	#
	# Process multiple stops
	#
	...
	for (j = 1; j <= split_num; j += 1)
		...
		#
			...
			if (match(arraye[j],floating_pt_num))
				...
				cents *= 0.01
				cents = convert_to_freq(cents)
				sub(SUBSEP,cents,arraye[j])
...
#	This function takes cbr input and translates to frequency
#
function process_cbr(data_token,    cbr,freq,return_token,arrayf,j,split_num)
	...
	for (j = 1; j <= split_num; j += 1)
		...
		#
			...
			cbr = 0
			#
			# If a valid cbr number exists, store and process it by 
			# converting it to frequency
			#
			if (match(arrayf[j],floating_pt_num))
				...
				#
				# The resulting value must not be less than 0
				#
				if (cbr < -4600 || cbr > 4600)
					{
					print "freq: ERROR: cbr value out of range in "\
						 "line " NR "." > stderr
					exit
					}
				freq = ((10 ^ (cbr / 21.4) - 1) / 4.37) * 1000
				freq = sprintf("%." precision "f",freq)
				sub(SUBSEP,freq,arrayf[j])
...
#	This function takes cocho input and translates to frequency
#
...
#	This function takes MIDI input and translates to frequency
#
function process_MIDI(data_token,    note,arrayk,j,return_token,split_num\
	...
	for (j = 1; j <= split_num; j += 1)
		...
		if (note > 0)
			{
			note -= 60
			return_token = return_token convert_to_freq(note) " "
			}
		}
	#
	# At least return a null token
...
#	This function takes fret input and translates to freq
#
		...
		for (j = 1; j <= split_num; j += 1)
			...
			if (arrayl[j] ~ /[^x:-]/)
				...
				if (!((temp_index,j) in relative))
					{
					print "freq: ERROR: No pitch specified for string "\
