...
#
#
#	This program is used to convert pitch units to solfg notes 
#
# Main Functions used:
...
#	Exchange_spines()		Process_data()			Convert_to_solfg()
#	To_lower()			To_upper()			Semits_to_solfg()
#	Process_pitch()		Process_Tonh()			Process_kern()
...
BEGIN {
	...
	pitch_array["G"] = 7;
	#
	solfg_array["A"] = "la" ; solfg_array["B"] = "si"; solfg_array["C"] = "do";
	solfg_array["D"] = "re" ; solfg_array["E"] = "mi"; solfg_array["F"] = "fa";
	solfg_array["G"] = "sol";
	#
...
##################################################################
##			FUNCTIONS USED IN SOLFG.AWK
##################################################################

##################################################################
##			Function Parse_command
#	This function checks that the input passed from solfg.ksh
	...
	for (j = 1; j <= current_no_of_spines; j += 1)
		...
		if ($j ~ /^\*\*/)
			...
			if ($j ~ pc_key_sig) check_pc_key($j,j)
				...
				{
				print "solfg: ERROR: Invalid key signature interpretation "\
		...
		# the output line must show solfg.
		#
		if ($j ~ input_interps) interp_line = interp_line "**solfg\t"
...
function get_absolute(interp,position,    temp_index)
	...
	if (interp !~ abs_tuning)
		{
		print "solfg: ERROR: Invalid absolute tuning interpretation in "\
			 "line " NR "." > stderr
		exit
		}
	#
...
function get_relative(interp,position,   courses,i,temp_index,num1,string_num)
	...
	if (interp !~ rel_tuning)
		{
		print "solfg: ERROR: Invalid relative tuning interpretation in "\
			 "line " NR "." > stderr
		exit
		}
	#
...
function get_frets(interp,position,   temp_index,temp_array,num1,frets,i)
	...
	if (interp !~ fret_tuning)
		{
		print "solfg: ERROR: Invalid fret tuning interpretation in "\
			 "line " NR "." > stderr
		exit
		}
	#
...
########################################################################
##				Function Convert_to_solfg
#	This function converts the note in current_note to a solfg value and
# returns it to the calling function.
#
function convert_to_solfg()
	{
	current_note[2] = solfg_array[current_note[2]]
	if (current_note[3] ~ /-/)
		{
		gsub("-","b",current_note[3])
		sub("b","~b",current_note[3])
		}
	else if (current_note[3] ~ /#/)
		{
		gsub("#","d",current_note[3])
		sub("d","~d",current_note[3])
		}
	else if (current_note[3] ~ /n/)
		{
		sub("n","~n",current_note[3])
		}
...
#############################################################################
##				Function Semits_to_solfg
#	This function converts semits to solfg notes
#
function semits_to_solfg(semits,spine, i,note,class,temp,deviation,pitch,arrayl)
	{
	if (semits+0 <= -0.5)
		{
		note = int(semits - .5)
	...
	return convert_to_solfg()
	}

#########################################################################
##				Function Process_pitch
#	This function accepts pitch input and translates to solfg 
#
...
#	This function takes Tonh input and translates to solfg
#
...
#	This function accepts kern input and translates to solfg
#
function process_kern(data_token,  return_token,arrayc,j,split_num,not_found,
									accidentals,original,new)
	{
	#
	# Process multiple stops
	...
	for (j = 1; j <= split_num; j += 1)
		...
		#
			...
			if (match(arrayc[j],/#+/))
				{
				accidentals = substr(arrayc[j],RSTART,RLENGTH)
				current_note[3] = accidentals
				sub(/#+/,"",arrayc[j])
				}
			else if (match(arrayc[j],/-+/))
				{
				accidentals = substr(arrayc[j],RSTART,RLENGTH)
				current_note[3] = accidentals
				sub(/-+/,"",arrayc[j])
...
#	This function takes semits input and translates to solfg
#
	...
	for (j = 1; j <= split_num; j += 1)
		...
		#
			...
			if (match(arrayg[j],floating_pt_num))
				...
				note = semits_to_solfg(note,spine)
...
#	This function takes cents input and translates to solfg
#
...
#	This function takes specC input and translates to solfg
#
		...
		#
			...
			if (match(arraye[j],floating_pt_num))
				...
				if (note > 0)
					...
					{
					print "solfg: ERROR: Invalid frequency value in "\
						 "line " NR "." > stderr
					exit
					}
				note = semits_to_solfg(note,spine)
...
#	This function takes frequency input and translates to solfg
#
...
#	This function takes MIDI input and translates to solfg
#
function process_MIDI(data_token,spine,   note,arrayi,j,return_token,split_num\
	...
	for (j = 1; j <= split_num; j += 1)
		...
		if (note > 0)
			{
			note -= 60
			return_token = return_token semits_to_solfg(note,spine) " "
			}
		}
	#
	# At least return a null token
...
#	This function takes fret input and translates to solfg
#
		...
		for (j = 1; j <= split_num; j += 1)
			...
			if (arrayl[j] ~ /[^x:-]/)
				...
				if (!((temp_index,j) in relative))
					{
					print "solfg: ERROR: No pitch specified for string "\
				...
				for (i = 1; i <= split(relative[temp_index,j],strings,",");\
					...
					if (semits != old_semits || return_token == "")
						return_token = return_token \
								semits_to_solfg(semits,position) " "
					old_semits = semits
					}
				}
			}
	...
	for (j = 1; j <= split_num; j += 1)
		...
		#
			...
			if (match(degree_array[1],/[1-7]/))
				...
				#
				return_token = return_token convert_to_solfg() " "
				}
			}
		}
	#
