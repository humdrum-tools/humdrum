...
#							stringed coursed in **fret spines
...
#
#
#	This program is used to convert all pitch units to **pitch notes 
#
# Main Functions used:
...
#	Exchange_spines()		Process_data()			Convert_to_pitch()
#	To_lower()			To_upper()			Semits_to_pitch()
#	Process_kern()			Process_Tonh()			Process_solfg()
...
BEGIN {
	...
	cont_tie_reg = "\\]|_"
	indicators = "^(\\*\\+|\\*\\-|\\*\\^|\\*v|\\*x)$"
	input_interps = "^(\\*\\*specC|\\*\\*kern|\\*\\*Tonh|\\*\\*solfg"\
...
##################################################################
##			FUNCTIONS USED IN PITCH.AWK
##################################################################

##################################################################
##			Function Parse_command
#	This function checks that the input passed from pitch.ksh
	...
	for (j = 1; j <= current_no_of_spines; j += 1)
		...
		if ($j ~ /^\*\*/)
			...
			if ($j ~ pc_key_sig) check_pc_key($j,j)
				...
				{
				print "pitch: ERROR: Invalid key signature interpretation "\
		...
		# the output line must show pitch.
		#
		if ($j ~ input_interps) interp_line = interp_line "**pitch\t"
...
function get_absolute(interp,position,    temp_index)
	...
	if (interp !~ abs_tuning)
		{
		print "pitch: ERROR: Invalid absolute tuning interpretation in "\
			 "line " NR "." > stderr
		exit
		}
	#
...
function get_relative(interp,position,   courses,i,temp_index,num1,string_num)
	...
	if (interp !~ rel_tuning)
		{
		print "pitch: ERROR: Invalid relative tuning interpretation in "\
			 "line " NR "." > stderr
		exit
		}
	#
...
function get_frets(interp,position,   temp_index,temp_array,num1,frets,i)
	...
	if (interp !~ fret_tuning)
		{
		print "pitch: ERROR: Invalid fret tuning interpretation in "\
			 "line " NR "." > stderr
		exit
		}
	#
...
#	This function converts the note in current_note to an pitch value and
# returns it to the calling function.
#
function convert_to_pitch(    return_token)
	{
	gsub("-","b",current_note[3])
	gsub("n","",current_note[3])
...
#############################################################################
##				Function Semits_to_pitch
#	This function converts semits to pitch notes
#
function semits_to_pitch(semits,spine,  i,note,class,temp,deviation,pitch,\
														arrayl)
	{
	if (semits+0 <= -0.5)
		{
...
#
function process_kern(data_token,  return_token,arraya,j,split_num,not_found,
	...
	for (j = 1; j <= split_num; j += 1)
		...
		#
		if (options ~ /t/ && arraya[j] ~ cont_tie_reg)
			{ if (split_num == 1) return_token = "." }
		#
		# Simply output any rests
		#
		else if (arraya[j] ~ /r/) return_token = return_token arraya[j] " "
		#
		...
		#
			...
			{
			original = arraya[j]
			if (options ~ /x/ && arraya[j] ~ tie_reg)
				sub(tie_reg,"",arraya[j])
			not_found = FALSE
			current_note[4] = ""
			if (match(arraya[j],/#+/))
				{
			...
			if (match(arraya[j],kern_pitch))
				{
				current_note[2] = substr(arraya[j],RSTART,1)
				...
				sub(kern_pitch,SUBSEP,arraya[j])
				}
			else not_found = TRUE
			if (not_found) return_token = return_token original " "
			else
				{
				new = convert_to_pitch()
...
#	This function takes Tonh input and translates to pitch
#
...
#	This function takes solfg input and translates to pitch
#
...
#	This function takes semits input and translates to pitch
#
	...
	for (j = 1; j <= split_num; j += 1)
		...
		#
			...
			if (match(arrayg[j],floating_pt_num))
				...
				note = semits_to_pitch(note,spine)
...
#	This function takes cents input and translates to pitch
#
...
#	This function takes specC input and translates to pitch
#
		...
		#
			...
			if (match(arraye[j],floating_pt_num))
				...
				if (note > 0)
					...
					{
					print "pitch: ERROR: Invalid frequency value in "\
						 "line " NR "." > stderr
					exit
					}
				note = semits_to_pitch(note,spine)
...
#	This function takes frequency input and translates to pitch
#
...
#	This function takes MIDI input and translates to pitch
#
function process_MIDI(data_token,spine,   note,arrayi,j,return_token,split_num\
	...
	for (j = 1; j <= split_num; j += 1)
		...
		if (note > 0)
			{
			note -= 60
			return_token = return_token semits_to_pitch(note,spine) " "
			}
		}
	#
	# At least return a null token
		...
		for (j = 1; j <= split_num; j += 1)
			...
			if (arrayl[j] ~ /[^x:-]/)
				...
				if (!((temp_index,j) in relative))
					{
					print "pitch: ERROR: No pitch specified for string "\
				...
				for (i = 1; i <= split(relative[temp_index,j],strings,",");\
					...
					if (semits != old_semits || return_token == "")
						return_token = return_token \
								semits_to_pitch(semits,position) " "
					old_semits = semits
					}
				}
			}
	...
	for (j = 1; j <= split_num; j += 1)
		...
		#
			...
			if (match(degree_array[1],/[1-7]/))
				...
				#
				return_token = return_token convert_to_pitch() " "
				}
			}
		}
	#
