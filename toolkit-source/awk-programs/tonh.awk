...
#
#
#	This program is used to convert pitch units to Tonh notes 
#
# Main Functions used:
#	Parse_command()		Store_new_interps()		Check_pc_key()
...
#	Exchange_spines()		Process_data()			Convert_to_Tonh()
#	To_lower()			To_upper()			Semits_to_Tonh()
#	Process_pitch()		Process_kern()			Process_solfg()
...
BEGIN {
	...
	cont_tie_reg = "\\]|_"
	indicators = "^(\\*\\+|\\*\\-|\\*\\^|\\*v|\\*x)$"
	input_interps = "^(\\*\\*specC|\\*\\*pitch|\\*\\*kern|\\*\\*solfg"\
...
##################################################################
##			FUNCTIONS USED IN TONH.AWK
##################################################################

##################################################################
##			Function Parse_command
#	This function checks that the input passed from tonh.ksh
	...
	for (j = 1; j <= current_no_of_spines; j += 1)
		...
		if ($j ~ /^\*\*/)
			...
			if ($j ~ pc_key_sig) check_pc_key($j,j)
				...
				{
				print "tonh: ERROR: Invalid key signature interpretation "\
		...
		# the output line must show Tonh.
		#
		if ($j ~ input_interps) interp_line = interp_line "**Tonh\t"
...
function get_absolute(interp,position,    temp_index)
	...
	if (interp !~ abs_tuning)
		{
		print "tonh: ERROR: Invalid absolute tuning interpretation in "\
			 "line " NR "." > stderr
		exit
		}
	#
...
function get_relative(interp,position,   courses,i,temp_index,num1,string_num)
	...
	if (interp !~ rel_tuning)
		{
		print "tonh: ERROR: Invalid relative tuning interpretation in "\
			 "line " NR "." > stderr
		exit
		}
	#
...
function get_frets(interp,position,   temp_index,temp_array,num1,frets,i)
	...
	if (interp !~ fret_tuning)
		{
		print "tonh: ERROR: Invalid fret tuning interpretation in "\
			 "line " NR "." > stderr
		exit
		}
	#
...
########################################################################
##				Function Convert_to_Tonh
#	This function converts the note in current_note to a Tonh value and
...
function convert_to_Tonh()
	...
	else if (current_note[2] == "B")
		{
		if (current_note[3] == "-") current_note[3] = ""
		else current_note[2] = "H"
		}
	gsub("-","es",current_note[3])
	gsub("#","is",current_note[3])
...
#############################################################################
##				Function Semits_to_Tonh
#	This function converts semits to Tonh notes
#
function semits_to_Tonh(semits,spine,  i,note,class,temp,deviation,pitch,arrayl)
	{
	if (semits+0 <= -0.5)
		{
		note = int(semits - .5)
	...
	return convert_to_Tonh()
	}

#########################################################################
##				Function Process_pitch
#	This function accepts pitch input and translates to Tonh 
#
...
#	This function accepts kern input and translates to pitch
#
function process_kern(data_token,  return_token,arrayb,j,split_num,not_found,
									accidentals,original,new)
	{
	#
	# Process multiple stops
	...
	for (j = 1; j <= split_num; j += 1)
		...
		#
		if (options ~ /t/ && arrayb[j] ~ cont_tie_reg)
			{ if (split_num == 1) return_token = "." }
		#
		# Simply output any rests
		#
		else if (arrayb[j] ~ /r/) return_token = return_token arrayb[j] " "
		#
		...
		#
			...
			{
			original = arrayb[j]
			if (options ~ /x/ && arrayb[j] ~ tie_reg)
				sub(tie_reg,"",arrayb[j])
			not_found = FALSE
			current_note[4] = ""
			if (match(arrayb[j],/#+/))
				{
			...
			gsub("n","",arrayb[j])
			if (match(arrayb[j],kern_pitch))
				{
				current_note[2] = substr(arrayb[j],RSTART,1)
				...
				sub(kern_pitch,SUBSEP,arrayb[j])
				}
			else not_found = TRUE
			if (not_found) return_token = return_token original " "
			else
...
#	This function takes solfg input and translates to Tonh
#
...
#	This function takes semits input and translates to Tonh
#
	...
	for (j = 1; j <= split_num; j += 1)
		...
		#
			...
			if (match(arrayg[j],floating_pt_num))
				...
				note = semits_to_Tonh(note,spine)
...
#	This function takes cents input and translates to Tonh
#
...
#	This function takes specC input and translates to Tonh
#
		...
		#
			...
			if (match(arraye[j],floating_pt_num))
				...
				if (note > 0)
					...
					{
					print "tonh: ERROR: Invalid frequency value in "\
						 "line " NR "." > stderr
					exit
					}
				note = semits_to_Tonh(note,spine)
...
#	This function takes frequency input and translates to Tonh
#
...
#	This function takes MIDI input and translates to Tonh
#
function process_MIDI(data_token,spine,   note,arrayi,j,return_token,split_num\
	...
	for (j = 1; j <= split_num; j += 1)
		...
		if (note > 0)
			{
			note -= 60
			return_token = return_token semits_to_Tonh(note,spine) " "
			}
		}
	#
	# At least return a null token
...
#	This function takes fret input and translates to Tonh
#
		...
		for (j = 1; j <= split_num; j += 1)
			...
			if (arrayl[j] ~ /[^x:-]/)
				...
				if (!((temp_index,j) in relative))
					{
					print "tonh: ERROR: No pitch specified for string "\
				...
				for (i = 1; i <= split(relative[temp_index,j],strings,",");\
					...
					if (semits != old_semits || return_token == "")
						return_token = return_token \
									semits_to_Tonh(semits,position) " "
					old_semits = semits
					}
				}
			}
	...
	for (j = 1; j <= split_num; j += 1)
		...
		#
			...
			if (match(degree_array[1],/[1-7]/))
				...
				#
				return_token = return_token convert_to_Tonh() " "
				}
			}
		}
	#
